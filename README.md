...
1.使用预训练模型权重，把以下代码放到main.py文件中，放进main函数里面的第二步“里面”，构建魔改模型后面，数据准备前面。并且去把trainer.py文件中的初始学习率lr0改小（0.0005，0.0001等），因为使用预训练权重在微调了。
...
A1.可能是我们给网络模型训练的分辨率太低了（我们之前一直给的640*640），所以下一步可以继续更改图像分辨率为1280或者1536。以及数据集的质量太差了，本次使用的VisDrone2019-MOT数据集分布不均，且大多是小目标。
